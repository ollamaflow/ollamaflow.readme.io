...
Virtual Ollama and OpenAI API endpoints that clients connect to. Each frontend:

* Maps to a specific hostname or acts as a catch-all (`*`)
* Defines which backend Ollama or OpenAI API instances to use
* Specifies required models for automatic synchronization
* Configures load balancing behavior and request handling

### 2. **Backends**

Physical Ollama or OpenAI compatible instances in your infrastructure. Each backend:
...
* Maintains model discovery and availability tracking for Ollama instances

### 3. **Models**

AI models that are automatically managed across your Ollama fleet (not available for OpenAI compatible endpoints):
...
Transform a single Ollama instance into a distributed cluster without changing client code. OllamaFlow maintains full API compatibility with Ollama and OpenAI.
...
* **Scale High-Density CPU Systems**: Leverage dense CPU architectures such as ARM and [Ampere](https://www.amperecomputing.com) 
